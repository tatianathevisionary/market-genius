# BTC Genius Dashboard

_Updated 2026-06-19T04:52:51+00:00 (hourly via launchd) — framework: [SIGNALS.md](../docs/SIGNALS.md)_

## `Trending` | A: 3/4 | B: 1/3

**BTC $62,491** | 5d -3.0% | vol 0.89x avg | -14.3% vs 50d MA ($72,933)

> BTC↔Nasdaq correlation broken (<0.2): moves are crypto-specific — weight ETF flows and sentiment over macro right now.

## Macro chain (5-day moves)

| Series | 5d % | Chain role |
|---|---|---|
| Brent | -8.9% | shock originator |
| US 10Y | -2.2% | transmission |
| DXY | +1.1% | denominator |
| Nasdaq fut | +2.7% | risk appetite |
| Gold | -1.4% | hedge competitor |
| **BTC** | **-3.0%** | subject |

Correlations (30d): BTC↔Nasdaq **-0.256** | BTC↔Gold **0.881** | BTC/Gold ratio 15.043

## A-score — macro pressure release

- ✅ A1 oil rolls over
- ✅ A2 yields peak
- ⬜ A4 dxy tops
- ✅ A5 equities stable
- ➖ A3 rate-hike odds: not in free data (check CME FedWatch manually)

## B-score — seller exhaustion

- ⬜ B1 capitulation volume
- ✅ B5 correlation recouples
- ⬜ B6 failed new low
- ➖ B2 funding / B3 ETF flows / B4 on-chain: Tier-2 feeds, not yet wired

## Support / Resistance (confluence-rated)

| Side | Level | Methods | Strength |
|---|---|---|---|
| resistance | $65,639 | R1 x1, R3 $65,000 | 2 |
| resistance | $64,972 | R1 x1, R3 $65,000 | 2 |
| resistance | $62,553 | R1 x1 | 1 |
| **price** | **$62,491** | | |
| support | $59,109 | R1 x1, R3 $60,000 | 2 |

## Reddit sentiment

- capitulation: 0
- bottom_calling: 0
- froth: 0
- macro_chain: 0
- rotation: 0
- _as of 2026-06-19T04:37:48+00:00_

## Recent TradingView alerts

- `2026-06-04T02:14:54+00:00` — entry_confluence
