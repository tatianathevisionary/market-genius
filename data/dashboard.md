# BTC Genius Dashboard

_Updated 2026-07-17T20:47:30+00:00 (hourly via launchd) — framework: [SIGNALS.md](../docs/SIGNALS.md)_

## `Basing` | A: 2/4 | B: 0/3

**BTC $64,036** | 5d +0.4% | vol 0.99x avg | +0.5% vs 50d MA ($63,711)

> Basing regime: arm the §3 breakout entry — trigger is the range high on >1.5x volume with 5d ETF inflows positive.

> BTC↔Nasdaq correlation broken (<0.2): moves are crypto-specific — weight ETF flows and sentiment over macro right now.

## Macro chain (5-day moves)

| Series | 5d % | Chain role |
|---|---|---|
| Brent | +16.0% | shock originator |
| US 10Y | -0.6% | transmission |
| DXY | -0.2% | denominator |
| Nasdaq fut | -4.2% | risk appetite |
| Gold | -2.2% | hedge competitor |
| **BTC** | **+0.4%** | subject |

Correlations (30d): BTC↔Nasdaq **0.153** | BTC↔Gold **0.395** | BTC/Gold ratio 15.948

## A-score — macro pressure release

- ⬜ A1 oil rolls over
- ✅ A2 yields peak
- ✅ A4 dxy tops
- ⬜ A5 equities stable
- ➖ A3 rate-hike odds: not in free data (check CME FedWatch manually)

## B-score — seller exhaustion

- ⬜ B1 capitulation volume
- ⬜ B5 correlation recouples
- ⬜ B6 failed new low
- ➖ B2 funding / B3 ETF flows / B4 on-chain: Tier-2 feeds, not yet wired

## Support / Resistance (confluence-rated)

| Side | Level | Methods | Strength |
|---|---|---|---|
| resistance | $71,986 | R1 x1 | 1 |
| resistance | $67,248 | R1 x1 | 1 |
| resistance | $64,972 | R1 x1, R3 $65,000 | 2 |
| **price** | **$64,036** | | |
| support | $59,109 | R1 x1, R3 $60,000 | 2 |
| support | $57,912 | R1 x2 | 2 |

## Reddit sentiment

- capitulation: 0
- bottom_calling: 0
- froth: 0
- macro_chain: 0
- rotation: 0
- _as of 2026-07-17T20:30:00+00:00_

## Recent TradingView alerts

- `2026-06-04T02:14:54+00:00` — entry_confluence
