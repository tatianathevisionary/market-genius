# BTC Genius Dashboard

_Updated 2026-09-23T08:55:31+00:00 (hourly via launchd) — framework: [SIGNALS.md](../docs/SIGNALS.md)_

## `Trending` | A: 2/4 | B: 1/3

**BTC $85,757** | 5d +12.2% | vol 1.19x avg | +15.7% vs 50d MA ($74,117)

> BTC↔Nasdaq correlation broken (<0.2): moves are crypto-specific — weight ETF flows and sentiment over macro right now.

## Macro chain (5-day moves)

| Series | 5d % | Chain role |
|---|---|---|
| Brent | -9.5% | shock originator |
| US 10Y | +0.0% | transmission |
| DXY | +1.1% | denominator |
| Nasdaq fut | +7.1% | risk appetite |
| Gold | -0.8% | hedge competitor |
| **BTC** | **+12.2%** | subject |

Correlations (30d): BTC↔Nasdaq **-0.039** | BTC↔Gold **0.1** | BTC/Gold ratio 19.698

## A-score — macro pressure release

- ✅ A1 oil rolls over
- ⬜ A2 yields peak
- ⬜ A4 dxy tops
- ✅ A5 equities stable
- ➖ A3 rate-hike odds: not in free data (check CME FedWatch manually)

## B-score — seller exhaustion

- ⬜ B1 capitulation volume
- ✅ B5 correlation recouples
- ⬜ B6 failed new low
- ➖ B2 funding / B3 ETF flows / B4 on-chain: Tier-2 feeds, not yet wired

## Support / Resistance (confluence-rated)

| Side | Level | Methods | Strength |
|---|---|---|---|
| **price** | **$85,757** | | |
| support | $82,262 | R1 x1 | 1 |
| support | $81,347 | R1 x1 | 1 |
| support | $76,248 | R1 x1 | 1 |

## Reddit sentiment

- capitulation: 0
- bottom_calling: 0
- froth: 0
- macro_chain: 0
- rotation: 0
- _as of 2026-09-23T08:48:52+00:00_

## Recent TradingView alerts

- `2026-06-04T02:14:54+00:00` — entry_confluence
