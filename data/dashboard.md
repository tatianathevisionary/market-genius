# BTC Genius Dashboard

_Updated 2026-07-08T09:39:16+00:00 (hourly via launchd) — framework: [SIGNALS.md](../docs/SIGNALS.md)_

## `Basing` | A: 1/4 | B: 1/3

**BTC $62,049** | 5d -0.8% | vol 1.13x avg | -5.9% vs 50d MA ($65,918)

> Basing regime: arm the §3 breakout entry — trigger is the range high on >1.5x volume with 5d ETF inflows positive.

> 1 froth/exit signal(s) active — see §4 before adding risk.

## Macro chain (5-day moves)

| Series | 5d % | Chain role |
|---|---|---|
| Brent | +8.1% | shock originator |
| US 10Y | +3.6% | transmission |
| DXY | -0.0% | denominator |
| Nasdaq fut | -5.1% | risk appetite |
| Gold | +1.1% | hedge competitor |
| **BTC** | **-0.8%** | subject |

Correlations (30d): BTC↔Nasdaq **0.412** | BTC↔Gold **0.801** | BTC/Gold ratio 15.254

## A-score — macro pressure release

- ⬜ A1 oil rolls over
- ⬜ A2 yields peak
- ✅ A4 dxy tops
- ⬜ A5 equities stable
- ➖ A3 rate-hike odds: not in free data (check CME FedWatch manually)

## B-score — seller exhaustion

- ⬜ B1 capitulation volume
- ✅ B5 correlation recouples
- ⬜ B6 failed new low
- ➖ B2 funding / B3 ETF flows / B4 on-chain: Tier-2 feeds, not yet wired

## Support / Resistance (confluence-rated)

| Side | Level | Methods | Strength |
|---|---|---|---|
| resistance | $71,986 | R1 x1 | 1 |
| resistance | $67,248 | R1 x1 | 1 |
| resistance | $64,972 | R1 x1, R3 $65,000, R4 50d MA | 3 |
| **price** | **$62,049** | | |
| support | $59,109 | R1 x1, R3 $60,000 | 2 |
| support | $57,912 | R1 x2 | 2 |

## Reddit sentiment

- capitulation: 0
- bottom_calling: 0
- froth: 0
- macro_chain: 0
- rotation: 0
- _as of 2026-07-08T09:35:29+00:00_

## Recent TradingView alerts

- `2026-06-04T02:14:54+00:00` — entry_confluence
