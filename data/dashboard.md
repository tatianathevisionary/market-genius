# BTC Genius Dashboard

_Updated 2026-07-26T07:57:30+00:00 (hourly via launchd) — framework: [SIGNALS.md](../docs/SIGNALS.md)_

## `Macro-driven selloff` | A: 0/4 | B: 0/3

**BTC $64,296** | 5d -3.3% | vol 0.53x avg | +1.7% vs 50d MA ($63,244)

> Cause still live, sellers not exhausted — no entry. Track A-score daily; oil and 10Y are the leads.

> 1 froth/exit signal(s) active — see §4 before adding risk.

> BTC↔Nasdaq correlation broken (<0.2): moves are crypto-specific — weight ETF flows and sentiment over macro right now.

## Macro chain (5-day moves)

| Series | 5d % | Chain role |
|---|---|---|
| Brent | +9.8% | shock originator |
| US 10Y | +3.0% | transmission |
| DXY | +0.7% | denominator |
| Nasdaq fut | -1.7% | risk appetite |
| Gold | +1.4% | hedge competitor |
| **BTC** | **-3.3%** | subject |

Correlations (30d): BTC↔Nasdaq **-0.249** | BTC↔Gold **0.404** | BTC/Gold ratio 15.794

## A-score — macro pressure release

- ⬜ A1 oil rolls over
- ⬜ A2 yields peak
- ⬜ A4 dxy tops
- ⬜ A5 equities stable
- ➖ A3 rate-hike odds: not in free data (check CME FedWatch manually)

## B-score — seller exhaustion

- ⬜ B1 capitulation volume
- ⬜ B5 correlation recouples
- ⬜ B6 failed new low
- ➖ B2 funding / B3 ETF flows / B4 on-chain: Tier-2 feeds, not yet wired

## Support / Resistance (confluence-rated)

| Side | Level | Methods | Strength |
|---|---|---|---|
| resistance | $74,959 | R1 x1, R3 $75,000 | 2 |
| resistance | $67,079 | R1 x2 | 2 |
| resistance | $64,972 | R1 x1, R3 $65,000 | 2 |
| **price** | **$64,296** | | |
| support | $59,109 | R1 x1, R3 $60,000 | 2 |
| support | $57,912 | R1 x2 | 2 |

## Reddit sentiment

- capitulation: 0
- bottom_calling: 0
- froth: 0
- macro_chain: 0
- rotation: 0
- _as of 2026-07-26T07:45:51+00:00_

## Recent TradingView alerts

- `2026-06-04T02:14:54+00:00` — entry_confluence
